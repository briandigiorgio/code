...
if __name__=='__main__':
    ...
    print("Loading files...")
    Mi25,Mie25,ad25,ade25=read_ad_mass('SPX-GAU-MILESHC-composite_0.25Re.fits',
        adv = True)
    Mi50,Mie50,ad50,ade50=read_ad_mass('SPX-GAU-MILESHC-composite_0.50Re.fits',
        adv = True)
    Mi75,Mie75,ad75,ade75=read_ad_mass('SPX-GAU-MILESHC-composite_0.75Re.fits',
        adv = True)
    Mi100,Mie100,ad100,ade100=read_ad_mass('SPX-GAU-MILESHC-composite_1.00Re.fits',
        adv = True)
    Mi125,Mie125,ad125,ade125=read_ad_mass('SPX-GAU-MILESHC-composite_1.25Re.fits',
        adv = True)

    #combines variables into easier to use arrays
    Miarray = np.asarray([Mi25,Mi50,Mi75,Mi100,Mi125])
    Miearray =  np.asarray([Mie25,Mie50,Mie75,Mie100,Mie125])
    ...
    quintiles = np.asarray(list(map(lambda x: np.percentile(x, 
        [20,40,60,80,100]), [Mi25,Mi50,Mi75,Mi100,Mi125])))
    print(quintiles)
    ...
    for i in range(5):
        ...
        q1cut = (Miarray[i]<quintiles[i][0])
        q2cut = (Miarray[i]>quintiles[i][0])*(Miarray[i]<quintiles[i][1])
        q3cut = (Miarray[i]>quintiles[i][1])*(Miarray[i]<quintiles[i][2])
        q4cut = (Miarray[i]>quintiles[i][2])*(Miarray[i]<quintiles[i][3])
        q5cut = (Miarray[i]>=quintiles[i][3])
    ...
    plt.grid(True)
    plt.title(plottype + ' AD Profiles for Decreasing Mi Quintiles')
    plt.xlabel('Re')
