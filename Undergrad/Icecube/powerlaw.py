...
def fit(data, ybins, curve,  bandsize = 0):
    ...
    #take the log of the data and bins to get a straight line
    #logbins, lognormed = np.log10([ybins, normed])
    ...
    #reassign errors on 0 data points
    for i in range(len(error)):
        if error[i] == 0.:
            error[i] = np.inf

    #return best fit line slope
    return sp.curve_fit(curve, bins,normed,sigma = error,p0=(37200,-3.92))[0][1]
...
if __name__ == "__main__":
    ...
    for pixel,zenbin in enumerate(binnum):
        ...
        if thetas[pixel] >= 2:
            ...
            if args.bandsize and not pavgs[zenbin]:
                ...
                pavgs[zenbin] = fit(average, ybins, curve, args.bandsize)
            ...
            pixfit = fit(matrix[pixel], ybins, curve)
    ...
    #show in normal IceCube orthview of southern hemisphere if not specified
        ...
        hp.orthview(dp, rot = [0,-90,180], half_sky = True, min = args.min,
